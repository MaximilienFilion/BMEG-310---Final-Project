...
# ==============================================================================
# INVESTIGATION 2: HIERARCHICAL Density-Based Clustering (HDBSCAN)
# ==============================================================================

## 3.2.1 Prepare Data for HDBSCAN
...
## 3.2.2 Apply HDBSCAN Clustering
# Perform density-based clustering
```{r inv4_hdbscan}
min_pts <- c(2, 5, 10, 15, 20, 25, 30, 35)
hdbscan_list <- list()

# Fit all models
for (i in seq_along(min_pts)) {
  hdbscan_list[[i]] <- hdbscan(gene_patient_matrix_reduced,
                               minPts = min_pts[i])
}
```

## 3.2.3 Visualize HDBSCAN Clusters
```{r inv4_visualize}
...
for (i in seq_along(min_pts)) {
  ...
  # Check for clusters and condensed tree
  num_clusters <- length(unique(res$cluster[res$cluster != 0]))
  tree_exists <- !is.null(res$condensedTree) && nrow(res$condensedTree) > 0
  
  if (num_clusters > 0 && tree_exists) {
    plot(res$condensedTree, main = paste("minPts =", min_pts[i]))
  } else {
    # Empty plot with message
    plot.new()
    title(main = paste("minPts =", min_pts[i]))
    text(0.5, 0.5, "No clusters found", cex = 1.2)
  }
}
...
for (i in seq_along(min_pts)) {
  cat("\n==== Results for minPts =", min_pts[i], "====\n")
  print(hdbscan_list[[i]])
  
## ChatGPT was used to help plot all the plots in the same image
}
```
...
# ==============================================================================
# INVESTIGATION 3: Patient Outcomes Across HDBSCAN Clusters
# ==============================================================================

## 3.3.1 Assign Patients to HDBSCAN Clusters
...
## 3.3.2 Kaplan-Meier Curves by HDBSCAN Cluster
...
## 3.3.3 ANOVA for Continuous Variables (HDBSCAN)
...
## 3.3.4 Chi-Square Tests for Categorical Variables (HDBSCAN)
...
## 3.3.5 Compare Hierarchical vs HDBSCAN Results
