...
top_20_genes_plot <- ggplot(top_20_genes, aes(x = reorder(Hugo_Symbol, -Frequency), y = Frequency)) +
...
ggsave(top_20_genes_plot, filename = "Top20NonsynonymouslyMutatedGenesinKIRC", device = "png", width = 7, height = 3, dpi = 300)
```
...
# Compute cut heights for 4 clusters
h_1 <- hc_1$height[length(hc_1$height) - 3]
h_2 <- hc_2$height[length(hc_2$height) - 3]

# Convert base R plots to ggplot
hc_1_plot <- as.ggplot(~{
  plot(hc_1, main="Ward.D Hierarchical Clustering of KIRC Patients",
       xlab="KIRC Patients", sub="", cex=0.8, labels=FALSE)
  abline(h = h_1, col = "red", lwd = 2, lty = 2)
})

hc_2_plot <- as.ggplot(~{
  plot(hc_2, main="Ward.D2 Hierarchical Clustering of KIRC Patients",
       xlab="KIRC Patients", sub="", cex=0.8, labels=FALSE)
  abline(h = h_2, col = "red", lwd = 2, lty = 2)
})
...
# Save
ggsave("HierarchicalClusteringWardD.png", hc_1_plot, width=7, height=3, dpi=300)
ggsave("HierarchicalClusteringWardD2.png", hc_2_plot, width=7, height=3, dpi=300)
...
```{r inv5_assign}
# Create a new data frame that has the disease-specific survival, overall survival, and all the clustering data
...
clinical_data_subset <- data_clinical_patient[
 ...
 c("PATIENT_ID", "DSS_STATUS", "DSS_MONTHS", "OS_STATUS", "OS_MONTHS", "AGE", "AJCC_PATHOLOGIC_TUMOR_STAGE")
]
...
```{r inv5_survival}
surv_obj_ward_DSS <- Surv(time = merged_data$DSS_MONTHS, event=merged_data$DSS_STATUS)

# KM plot for ward_D1 clustering
ward_d1_DSS <- surv_fit(surv_obj_ward_DSS ~ clusters_ward_D1, data = merged_data)
ward_d1_DSS_plot <- ggsurvplot(ward_d1_DSS,
           data = merged_data,
           pval = TRUE,
           conf.int = TRUE,
...
ward_d2_DSS <- surv_fit(surv_obj_ward_DSS ~ clusters_ward_D2, data = merged_data)
ward_d2_DSS_plot <- ggsurvplot(ward_d2_DSS,
           data = merged_data,
           pval = TRUE,
           conf.int = TRUE,
           title = "Disease Specific Survival for Identified Ward.D2 Hierarchical Cluster",
           xlab = "Time (Months)",
           ylab = "Survival Probability",
           ggtheme = theme_minimal(base_size = 14))
#Plot
...
# Overall survival 
surv_obj_ward_OS <- Surv(time = merged_data$OS_MONTHS, event=merged_data$OS_STATUS)

# KM plot for ward_D1 clustering
ward_d1_OS <- surv_fit(surv_obj_ward_OS ~ clusters_ward_D1, data = merged_data)
ward_d1_OS_plot <- ggsurvplot(ward_d1_OS,
           data = merged_data,
           pval = TRUE,
           conf.int = TRUE,
           title = "Overall Survival for Identified Ward.D1 Hierarchical Cluster",
           xlab = "Time (Months)",
           ylab = "Survival Probability",
           ggtheme = theme_minimal(base_size = 14))

# KM plot for ward_D2 clustering
ward_d2_OS <- surv_fit(surv_obj_ward_OS ~ clusters_ward_D2, data = merged_data)
ward_d2_OS_plot <- ggsurvplot(ward_d2_OS,
           data = merged_data,
           pval = TRUE,
           conf.int = TRUE,
           title = "Overall Survival for Identified Ward.D2 Hierarchical Cluster",
           xlab = "Time (Months)",
           ylab = "Survival Probability",
           ggtheme = theme_minimal(base_size = 14))
...
# Save the plots
ggsave("DSSHierarchicalClusteringWardD.png", ward_d1_DSS_plot$plot, width=7, height=3, dpi=300)
ggsave("DSSHierarchicalClusteringWardD2.png", ward_d2_DSS_plot$plot, width=7, height=3, dpi=300)
ggsave("OSHierarchicalClusteringWardD.png", ward_d1_OS_plot$plot, width=7, height=3, dpi=300)
ggsave("OSHierarchicalClusteringWardD2.png", ward_d2_OS_plot$plot, width=7, height=3, dpi=300)
```
...
merged_data$clusters_ward_D1 <- as.factor(merged_data$clusters_ward_D1)
merged_data$clusters_ward_D2 <- as.factor(merged_data$clusters_ward_D2)

anova_result_Ward_D1 <- aov(merged_data$AGE ~ clusters_ward_D1, data = merged_data)
summary(anova_result_Ward_D1)

anova_result_Ward_D2 <- aov(merged_data$AGE ~ clusters_ward_D2, data = merged_data)
summary(anova_result_Ward_D2)

TukeyHSD(anova_result_Ward_D1)
TukeyHSD(anova_result_Ward_D2)

# Plot the Anova test results
anova_ward_d1_plot <- ggplot(merged_data, aes(x = clusters_ward_D1, y = AGE, fill = clusters_ward_D1)) +
  geom_boxplot() +
  labs(
    title = "Patient Age Distribution Across Ward.D1 Clusters",
    x = "Cluster",
    y = "Age (years)"
  ) +
  theme_minimal(base_size = 14) +
...
# [INSERT CODE HERE]
```
>>>>>>> 522abae87f0b04a90302f63b3b085ddb3fbd85c0
