...
# Plot the bar graph
ggplot(stage_summary, aes(x = STAGE, y = percentage)) +
  geom_bar(stat = "identity", fill = "skyblue", color = "black") +
  geom_text(aes(label = label), vjust = -0.5, size = 4) +  # Add count labels above bars
  labs(
    title = "Percentage of Patients by Tumor Stage",
    x = "Tumor Stage",
    y = "Percentage (%)"
  ) +
  theme_minimal(base_size = 14) +
  ylim(0, max(stage_summary$percentage) * 1.1)
...
p1 <- plot_donut(data_clinical_patient, "SEX", "Sex Distribution")
p2 <- plot_donut(data_clinical_patient, "ETHNICITY", "Ethnicity Distribution")
p3 <- plot_donut(data_clinical_patient, "RACE", "Race Distribution")
p4 <- plot_donut(data_clinical_patient, "GENETIC_ANCESTRY_LABEL", "Ancestry Distribution")
...
```
```{r}
#Extracted table of just top 5 genes to save space:
top5 <- survival_results %>%
  arrange(PValue) %>%
  head(5)
